...
var $getItems = function() {
	$.ajax({
		...
		success : function(data) {
			...
			$("#suggestions").empty();
			$("#t1").tmpl(data.oList).appendTo("#suggestions");
			$paging();
		},
		error : function() {
			alert("数据获取失败");
...
$("#audit_submit").click(function() {
	...
	var opinionContent = $("#appContent").text();
	...
	if(temp == 1) {
		...
		layer.confirm("本次审批结果为:批准<br/>请确认信息<br/>用户：" +
			username + "<br/>建议内容:" + opinionContent, {
				btn: ['确认提交', '再审核一会'], //按钮
				shade: false //不显示遮罩
			},
			function() {
				//处理业务
				$handleOneSuggestion(appId, 1, "");
			},
			function() {});
	} else if(temp == 2) {
		if(rejectReason != "") {
			...
			layer.confirm("本次审批结果为:驳回<br/>请确认信息<br/>用户：" +
				...
				function() {
					//处理业务
					$handleOneSuggestion(appId, 2, rejectReason);
				},
				function() {});
		} else {
			layer.msg('请输入驳回理由');
...
//提交
var $handleOneSuggestion = function(sid, status, reason) {
	$.ajax({
		type : "post",
		url : "handleOneSuggestion",
		async : true,
		dataType : "json",
		data :{
			id : sid,
...
$(document).on('click', '.approve', function() {
	...
	var appContent = $(this).parent().parent().children("td:eq(4)").text();
	$("#appId").text(appId);
